...
| `CONSENT_UI_URL`      | Optional. A dedicated consent UI. Unset → `auth-ui` handles consent; set → this UI does. Identity + JWKS derive from it, like `AUTH_UI_URL`.                                     |
...
- **Interactions, not a fixed app.** An authorization has required interactions (`authenticate`, `consent`); the AS drives whichever app serves each. With only `AUTH_UI_URL` set, `auth-ui` runs both. With `CONSENT_UI_URL` also set, `auth-ui` authenticates and a dedicated consent UI runs consent (and a separate consent service becomes the consent system-of-record, synced from the grant lifecycle). A client marked `first_party=true` in Authlete skips consent.
